...
impl<'a, G: Group, R: RngCore + CryptoRng> Encryption<'a, G, R> {
    ...
    pub fn encrypt(&mut self, public_key: &PublicKey<G>, message: &Message<G>) -> Ciphertext<G> {
        ...
        // we do not expose the randomness to the user. the randomness could be misunderstood and stored together with the ciphertext, even in cases where it is not needed (e.g., no decryption using the randomness)
